...
class Ui:
    ...
    def draw(
        ...
        diss_cap = format_number_with_suffix(
            sim.preview_vent_capacity + sim.preview_outlet_capacity, max_decimals=1
        )
        label = f"-{vent_amt} Heat (-{vent_rate}/t, cap {diss_cap}/t)"
        ...
        if tab_slots:
            ...
            for idx, slot in enumerate(tab_slots):
                ...
                if selected and pressed is not None and not locked:
                    ...
                    tex = hover
                tint = Color(140, 140, 140, 255) if locked else Color(255, 255, 255, 255)
    ...
    def draw_statistics_panel(self, sim: Simulation, layout: Layout) -> None:
        ...
        lines = [
            ...
            f"Vent dissipation cap: {fmt(sim.preview_vent_capacity, max_decimals=1)}/t",
            f"Outlet transfer cap: {fmt(sim.preview_outlet_capacity, max_decimals=1)}/t",
            f"Outlet + vent cap: {fmt(sim.preview_vent_capacity + sim.preview_outlet_capacity, max_decimals=1)}/t",
            "",
